...
function encountersFrom(previous: Round[]): Map<string, number> {
...
function costFromEncounters(groups: string[][], encounters: Map<string, number>, points: Record<string, number>): number {
...
export function pairCost(groups: string[][], previous: Round[], points: Record<string, number>): number {
  return costFromEncounters(groups, encountersFrom(previous), points)
}

/** Deterministic multi-shuffle optimiser. Repeats dominate score proximity in its cost. */
export function makePairings(players: Player[], previous: Round[], points: Record<string, number>, seed: number, maxTables?: number): string[][] {
  const sizes = tableSizes(players.length, maxTables); let best: string[][] = []; let bestCost = Infinity
  const ranked = [...players].sort((a, b) => (points[b.id] ?? 0) - (points[a.id] ?? 0))
  const encounters = encountersFrom(previous)
  for (let attempt = 0; attempt < 160; attempt++) {
    ...
    const cost = costFromEncounters(groups, encounters, points)
