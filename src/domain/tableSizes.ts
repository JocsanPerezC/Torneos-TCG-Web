...
export function tableSizes(playerCount: number, maxTables = Number.POSITIVE_INFINITY): number[] {
  if (!Number.isInteger(playerCount) || playerCount < 1)
    throw new Error('Se requiere al menos un jugador activo.');
  if (maxTables !== Number.POSITIVE_INFINITY && (!Number.isInteger(maxTables) || maxTables < 1))
    throw new Error('El máximo de mesas debe ser al menos uno.');
  if (Math.ceil(playerCount / 5) > maxTables)
    throw new Error('El máximo de mesas no permite distribuir a todos los jugadores.');
