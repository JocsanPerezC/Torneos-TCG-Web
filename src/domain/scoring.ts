...
export function isValidPodResult(pod: Pod): boolean {
  ...
  if (
    ...
    !pod.resultType ||
    results.some((r) => r.kills < 0 || !Number.isInteger(r.kills) || r.position < 1 || !Number.isInteger(r.position))
  )
    return false;
  if (
...
export function standings(tournament: Tournament): Standing[] {
  const stat = Object.fromEntries(
    tournament.players.map((player) => [
      player.id,
      { points: 0, wins: 0, kills: 0, roundsPlayed: 0, opponents: [] as string[] },
    ]),
  );
    ...
    for (const pod of round.pods)
      if (isValidPodResult(pod)) {
        ...
        for (const result of pod.results ?? []) {
          ...
          entry.points += points[result.playerId];
