...
export function isValidPodResult(pod: Pod): boolean {
  ...
  if (results.length !== pod.playerIds.length || results.some(r => r.points < 0 || !Number.isInteger(r.points) || r.kills < 0 || !Number.isInteger(r.kills))) return false
...
export function standings(tournament: Tournament): Standing[] {
  const stat = Object.fromEntries(tournament.players.map(player => [player.id, { points: 0, kills: 0, roundsPlayed: 0, opponents: [] as string[] }]))
  for (const round of tournament.rounds) for (const pod of round.pods) if (isValidPodResult(pod)) {
    for (const result of pod.results ?? []) {
      ...
      entry.points += result.points; entry.kills += result.kills; entry.roundsPlayed++
  ...
  rows.sort((a, b) => b.points - a.points || b.opponentStrength - a.opponentStrength || b.kills - a.kills || a.player.tieBreaker - b.player.tieBreaker)
