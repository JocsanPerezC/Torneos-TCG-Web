...
import { CircleX, Database, Equal, Eye, EyeOff, ExternalLink, Info, LogOut, Play, RefreshCw, ShieldCheck, Square, Trash2, Trophy, Users, type LucideIcon } from 'lucide-react'
...
function Header() {
  ...
  return <header className="app-header app-shell-header">
    <div className="app-shell-header__bar mx-auto max-w-6xl">
      <Link to="/" className="app-shell-header__brand"><img src="/tcg-tournament-icon.svg" alt="" />EDH <span>Tournaments</span></Link>
...
function Dashboard() {
  ...
  return <Layout><div className="dashboard-board">
    <header className="dashboard-board__header"><div><OrganizerIdentity user={user}/><p className="dashboard-board__eyebrow">{i18n.t('app.dashboard.title')}</p><h1>{i18n.t('app.dashboard.title')}</h1></div><button className={primary} onClick={() => setCreating(true)}>✦ {i18n.t('app.dashboard.newTournament')}</button></header>
    ...
    {(['activo', 'finalizado'] as const).map((status, index) => {
      const sectionTournaments = ownTournaments.filter(tournament => tournament.status === status)
      return <section key={status} className="dashboard-shelf"><header><span>0{index + 1}</span><h2>{status === 'activo' ? i18n.t('app.dashboard.active') : i18n.t('app.dashboard.finished')}</h2></header><div className="dashboard-shelf__grid">
        ...
        {ready && sectionTournaments.length === 0 && <div className="app-empty-state"><span aria-hidden>⌁</span><p>{i18n.t('app.dashboard.noTournaments')}</p>{status === 'activo' && <button className={primary} onClick={() => setCreating(true)}>{i18n.t('app.dashboard.newTournament')}</button>}</div>}
      </div></section>
    })}
...
function TournamentLayout() {
  ...
  const tabs = [['', '✦', i18n.t('app.tournament.overview')], ['players', '♙', i18n.t('app.tournament.players')], ['rounds', '⌁', i18n.t('app.tournament.rounds')], ['standings', '♛', i18n.t('app.tournament.standings')], ['statistics', '✹', i18n.t('app.statistics.title')], ['settings', '⚙', i18n.t('app.tournament.settings')]]
  return <Layout><header className="tournament-masthead"><div><p>{tournament.format}</p><h1>{tournament.name}</h1></div><Badge>{tournament.status}</Badge></header><nav className="tournament-tabs" aria-label={i18n.t('landing.navigation.label')}>{tabs.map(([suffix, symbol, label]) => <NavLink end={!suffix} className={({ isActive }) => `tournament-tabs__tab ${isActive ? 'is-active' : ''}`} key={label} to={`${base}${suffix ? `/${suffix}` : ''}`}><span aria-hidden>{symbol}</span>{label}</NavLink>)}</nav><Routes><Route index element={<Overview />} /><Route path="players" element={<Players />} /><Route path="rounds" element={<Rounds />} /><Route path="standings" element={<Standings />} /><Route path="statistics" element={<Statistics />} /><Route path="settings" element={<Settings />} /></Routes></Layout>
}
...
function Overview() { const tournament = Current(); const current = tournament.rounds.at(-1); return <section className="overview-board"><header><p>{i18n.t('app.tournament.state')}</p><h2>{i18n.t('app.tournament.overview')}</h2></header><div className="overview-board__metrics"><Metric label={i18n.t('app.tournament.activePlayers')} value={tournament.players.filter(player => player.active).length} symbol="♙" /><Metric label={i18n.t('app.tournament.rounds')} value={`${tournament.rounds.length}/${tournament.plannedRounds}`} symbol="⌁" /><Metric label={i18n.t('app.tournament.currentTable')} value={current ? `R${current.number}` : '—'} symbol="✦" /></div><TournamentInformation /></section> }
function Statistics() { return <BattleLedger statistics={tournamentStatistics(Current())} /> }
function TournamentInformation() { const tournament = Current(); const { id } = useParams(); const { updateTournament } = useTournaments(); const [information,setInformation] = useState(tournament.information); const [saved,setSaved] = useState(false); function save(event:FormEvent<HTMLFormElement>){event.preventDefault();updateTournament(id!,{information:information.trim()});setSaved(true);showToast(i18n.t('app.tournament.informationSaved'))} return <section className="notice-board"><header><span aria-hidden>⌁</span><div><h2>{i18n.t('app.tournament.information')}</h2><p>{i18n.t('app.tournament.informationHelp')}</p></div></header><form onSubmit={save}><textarea value={information} onChange={event=>{setInformation(event.target.value);setSaved(false)}} disabled={tournament.status==='finalizado'} maxLength={2000} rows={7} className={`${field} resize-y`} placeholder={i18n.t('app.tournament.informationPlaceholder')} /><div className="notice-board__actions"><span>{information.length}/2000</span><button disabled={tournament.status==='finalizado'} className={primary}>{i18n.t('app.tournament.saveInformation')}</button></div>{saved&&<p className="notice-board__saved">{i18n.t('app.tournament.informationSaved')}</p>}</form></section> }
function Metric({ label, value, symbol }: { label: string; value: string | number; symbol: string }) { return <article className="app-stat-pill"><span aria-hidden>{symbol}</span><strong>{value}</strong><small>{label}</small></article> }
function Players() {
