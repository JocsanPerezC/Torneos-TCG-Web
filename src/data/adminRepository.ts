...
export async function loadAdminOverview(): Promise<AdminOverview> {
  ...
  const [profilesResponse, tournamentsResponse] = await Promise.all([
    ...
    db
      .from('tournaments')
      .select(
        'id, owner_id, name, format, status, is_public, created_at, players(count), rounds(count)',
      )
      .order('created_at', { ascending: false }),
  ]);
  ...
  return {
    profiles: rows(profilesResponse.data).map((row) => ({
      ...
      role:
        row.role === 'super_admin' ? 'super_admin' : row.role === 'admin' ? 'admin' : 'organizer',
...
export async function updateProfileRole(profileId: string, role: AssignableRole) {
  const { error } = await client().rpc('set_profile_role', {
