...
const mapTournament = (row: Row): Tournament => {
  ...
  return {
    ...
    scoring: { first: Number(scoringRow.first_place ?? 3), second: Number(scoringRow.second_place ?? 2), third: Number(scoringRow.third_place ?? 1), fourth: Number(scoringRow.fourth_place ?? 0), tie: Number(scoringRow.tie_points ?? 2) },
...
export async function persistTournamentSnapshot(t: Tournament) { const { error } = await client().rpc('save_tournament_snapshot', { snapshot: t }); if (error) throw error }
